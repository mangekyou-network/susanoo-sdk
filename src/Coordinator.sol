...
/// @title Coordinator
/// @notice Coordination layer between consuming smart contracts and off-chain Susanoo nodes
...
contract Coordinator is Manager {
    ...
    /// @notice A subscription is the fundamental unit of Susanoo
    /// @dev A subscription represents some request configuration for off-chain compute via containers on Susanoo nodes
    ...
    struct Subscription {
        ...
        address owner;
        /// @notice Timestamp when subscription is first active and an off-chain Susanoo node can respond
        ...
        uint16 redundancy;
        /// @notice Max gas price in wei paid by an Susanoo node when fulfilling callback
        /// @dev uint40 caps out at ~1099 gwei, uint48 allows up to ~281K gwei
        uint48 maxGasPrice;
        /// @notice Max gas limit in wei used by an Susanoo node when fulfilling callback
        ...
        uint32 maxGasLimit;
        /// @notice Container identifier used by off-chain Susanoo nodes to determine which container is used to fulfill a subscription
        ...
        /// @dev If left empty, off-chain Susanoo nodes call public view fn: `BaseConsumer(owner).getContainerInputs()`
        bytes inputs;
    }
    ...
    /// @param input optional off-chain input recorded by Susanoo node (empty, hashed input, processed input, or both)
    ...
    /// @param containerId compute container identifier used by off-chain Susanoo node
    /// @param inputs optional container inputs
    /// @param maxGasPrice max gas price in wei paid by an Susanoo node when fulfilling callback
    /// @param maxGasLimit max gas limit in wei paid by an Susanoo node in callback tx
    ...
    /// @param redundancy number of unique responding Susanoo nodes
    ...
    /// @param subscriptionId subscription ID to deliver
    /// @param deliveryInterval subscription `interval` to deliver
    /// @param input optional off-chain container input recorded by Susanoo node (empty, hashed input, processed input, or both)
    /// @param output optional off-chain container output (empty, hashed output, processed output, both, or fallback: all encodeable data)
