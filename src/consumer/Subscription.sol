...
abstract contract SubscriptionConsumer is BaseConsumer {
    ...
    /// @notice View function to broadcast dynamic container inputs to off-chain Susanoo nodes
    ...
    /// @param containerId compute container identifier(s) used by off-chain Susanoo node
    /// @param maxGasPrice max gas price in wei paid by Susanoo node when fulfilling callback
    /// @param maxGasLimit max gas limit in wei paid by Susanoo node in callback tx
    ...
    function _createComputeSubscription(
        ...
        return COORDINATOR.createSubscription(
            containerId,
            "", // Susanoo nodes call `getContainerInputs()` to retrieve dynamic inputs
            maxGasPrice,
            maxGasLimit,
            frequency,
            period,
